...
import { TorontoTippyProps } from '../types';

const TorontoTippy: React.FC<TorontoTippyProps> = ({
  appendTarget,
  ...
  function getItem(linkTo: string, text: string) {
    return (
  ...
  function render() {
    ...
    return (
      <Tippy
        ...
        theme="toronto"
        appendTo={() => {
          return appendTarget.current || ({} as Element); // ts hack
        }}
        arrow={roundArrow}
        inertia={true}
        interactive={true}
...
export const Header = () => {
  ...
  function getLink(linkTo: string, buttonText: string) {
    return (
